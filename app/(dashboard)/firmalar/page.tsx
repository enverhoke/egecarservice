...
function FirmalarInner() {
  ...
  const filtered = useMemo(() => {
    return items.filter((x) =>
      [x.name, x.managerName, x.phone].join(' ').toLowerCase().includes(search.toLowerCase())
    );
  }, [items, search]);
        ...
        <form className="panel-card" onSubmit={saveFirm}>
          ...
          <div className="form-grid">
            ...
            <label className="field">
              <span>Durum</span>
              <select
                value={form.active ? 'Aktif' : 'Pasif'}
                onChange={(e) => setForm({ ...form, active: e.target.value === 'Aktif' })}
              >
                <option>Aktif</option>
                <option>Pasif</option>
              </select>
